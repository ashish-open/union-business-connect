...
// Sales — one route, the sub-nav switches the document type.
//
// This is where Collections went: invoices, payment links and chasing are all
// the sales side of the same ledger, and keeping them apart made the owner
// visit two screens to answer one question.
...
import { convert, Doc, DOC_SPEC, DocKind, docTotals, newDoc, SALES_KINDS } from "@/lib/docs";
...
const salesItems = [
  ...SALES_KINDS.map((k) => ({ label: DOC_SPEC[k].plural, href: `/sales/${k}` })),
  // Not a document type — but it is how the money comes in, so it belongs
  // beside the documents rather than in a rail of its own.
  { label: "Payment links", href: "/collections" },
];
...
export default function SalesPage() {
  ...
  if (!spec || spec.side !== "sales") {
    return (
      <AppShell>
        <SectionLayout title="Sales" items={salesItems} active={`/sales/${params.kind}`}>
          <Card>
  ...
  return (
    <AppShell>
      <SectionLayout title="Sales" items={salesItems} active={`/sales/${kind}`}>
        ...
        {spec.postsToLedger && mine.length > 0 && (
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <Card>
              ...
              <p className="mt-1.5 text-[11.5px] text-ink-3">
                {overdue.length > 0
                  ? `${formatINR(overdue.reduce((s, d) => s + docTotals(d).outstanding, 0))} to chase`
                  : "Nothing late"}
              </p>
            </Card>
          </div>
